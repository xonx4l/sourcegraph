...
func (s *Server) doClone(
	...
	output, err := executil.RunRemoteGitCommand(ctx, wrCmd, remoteURL, true, pw)
...
func setHEAD(ctx context.Context, logger log.Logger, rcf *wrexec.RecordingCommandFactory, repoName api.RepoName, dir common.GitDir, syncer vcssyncer.VCSSyncer, remoteURL *vcs.URL) error {
	...
	wrCmd := rcf.WrapWithRepoName(ctx, logger, repoName, cmd).WithRedactorFunc(r.Redact)
	output, err := executil.RunRemoteGitCommand(ctx, wrCmd, remoteURL, true, nil)
	if err != nil {

...
func (s *gitRepoSyncer) IsCloneable(ctx context.Context, repoName api.RepoName, remoteURL *vcs.URL) error {
	...
	wrCmd := s.recordingCommandFactory.WrapWithRepoName(ctx, log.NoOp(), repoName, cmd).WithRedactorFunc(r.Redact)
	out, err := executil.RunRemoteGitCommand(ctx, wrCmd, remoteURL, true, nil)
	if err != nil {
...
func (s *gitRepoSyncer) Fetch(ctx context.Context, remoteURL *vcs.URL, repoName api.RepoName, dir common.GitDir, _ string) ([]byte, error) {
	...
	r := urlredactor.New(remoteURL)
	wrCmd := s.recordingCommandFactory.WrapWithRepoName(ctx, log.NoOp(), repoName, cmd).WithRedactorFunc(r.Redact)
	output, err := executil.RunRemoteGitCommand(ctx, wrCmd, remoteURL, configRemoteOpts, nil)
	if err != nil {
